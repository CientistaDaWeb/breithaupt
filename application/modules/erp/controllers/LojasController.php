...
class Erp_LojasController extends Erp_Controller_Action {

    public function init() {
        $this->model = new Lojas_Model();
        $this->form = WS_Form_Generator::generateForm('Lojas', $this->model->getFormFields());
        parent::init();
    }
        ...
        if ($this->_request->isPost()):
            if ($this->form->isValid($this->_request->getPost())) :
                if ($_FILES['upload']['size'] > 0) {
                    ...
                    if ($upload->uploaded) {
                        $upload->Process(UPLOAD_PATH . '/lojas');
                        if ($upload->processed) {
